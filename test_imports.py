...
def delete_transaction(txn_id: int, db: Session):
    # 1. Locate the row
    obj = db.query(Account).filter(Account.id == txn_id).first()

    if obj:
        # 2. Delete the object
        db.delete(obj)
        # 3. Commit the change to the .db file
        db.commit()
        return True
    return False

if __name__ == "__main__":
    delete_transaction(3, Session())