...
function barClass(status) { return `bar-${status}`; }
  ...
  return {
    ...
    async syncAll() {
      ...
      try {
        ...
        const total = (res.results || []).reduce((s, r) => s + (r.imported || 0), 0);
...
function dashboardApp() {
  return {
    ...
    get totalIncome() { return this.currentMonthData?.income ?? 0; },
    get totalExpenses() { return this.currentMonthData?.expenses ?? 0; },
    get netThisMonth() { return this.totalIncome - this.totalExpenses; },
    ...
    renderCategoryPie() {
      ...
      const colors = ['#3b7eff','#f97316','#8b5cf6','#ec4899','#10b981','#f59e0b','#06b6d4','#94a3b8'];
      this.charts.pie = new Chart(el, {
        type: 'doughnut',
        data: {
          labels: data.map(r => r.category),
          datasets: [{ data: data.map(r => r.total), backgroundColor: colors, borderWidth: 0, hoverOffset: 4 }],
        },
        options: {
          plugins: {
    ...
    renderIncomeBar() {
      ...
      this.charts.bar = new Chart(el, {
        ...
        data: {
          ...
          datasets: [
            { label: 'Income', data: data.map(r => r.income), backgroundColor: 'rgba(22,163,74,0.75)', borderRadius: 4, borderSkipped: false },
            { label: 'Expenses', data: data.map(r => r.expenses), backgroundColor: 'rgba(220,38,38,0.7)', borderRadius: 4, borderSkipped: false },
          ],
        },
        options: {
    ...
    fmt, fmtAbs, barWidth, barClass, badgeClass,
  };
}
...
function transactionsApp() {
  return {
    ...
    navMonth(dir) {
      this.month_year = dir === -1 ? prevMonth(this.month_year) : nextMonth(this.month_year);
      this.load();
    },

    get monthLabel() { return monthLabel(this.month_year); },

    startEdit(txn) { this.editingId = txn.id; this.editCategory = txn.category; },
    cancelEdit() { this.editingId = null; },

    async saveEdit(txn) {
      await api.patch(`/api/transactions/${txn.id}`, { category: this.editCategory });
      txn.category = this.editCategory;
      this.editingId = null;
    },

    get totalExpenses() {
      return this.transactions.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0);
    },
    get totalIncome() {
  ...
  return {
    ...
    async connectBank() {
      ...
      if (!this.status?.configured) {
        this.error = 'Plaid credentials not configured. See the setup instructions below.';
        return;
      }
      try {
    ...
    _openLink(token) {
      const handler = Plaid.create({
        ...
        onExit: err => {
          if (err) this.error = err.display_message || err.error_message || 'Link exited with error';
        },
      });
      handler.open();
    },
    ...
    async syncAll() {
      this.syncing = true; this.syncResults = null; this.error = null;
      try {
        const res = await api.post('/api/plaid/sync-all', {});
    ...
    async syncOne(itemId) {
      this.syncing = true; this.error = null;
      try {
    ...
    async disconnect(itemId, name) {
      if (!confirm(`Disconnect ${name}? Your existing transactions will be kept.`)) return;
...
function importApp() {
  return {
    form: { account_name: '', account_type: 'checking', institution: '', bank_profile: 'sofi', run_ai: true },
    file: null,
    result: null,
    error: null,
    loading: false,
    ...
    async submit() {
      if (!this.file) { this.error = 'Please select a CSV file.'; return; }
      if (!this.form.account_name) { this.error = 'Account name is required.'; return; }
...
function budgetsApp() {
  return {
    ...
    categories: [],
    month_year: currentMonthYear(),
    form: { category: '', month_year: '', limit_amount: '', alert_threshold: 0.8 },
    showForm: false,
    editId: null,
    chart: null,

    async init() {
      this.categories = (await api.get('/api/transactions/categories')).map(c => c.name);
      await this.load();
    },
    ...
    renderChart() {
      ...
      this.chart = new Chart(el, {
        type: 'bar',
        data: {
          labels: items.map(b => b.category.length > 14 ? b.category.slice(0,14)+'…' : b.category),
          datasets: [
            { label: 'Spent', data: items.map(b => b.actual_amount), backgroundColor: items.map(b => b.status === 'over' ? 'rgba(220,38,38,0.75)' : b.status === 'warning' ? 'rgba(245,158,11,0.75)' : 'rgba(59,126,255,0.7)'), borderRadius: 4, borderSkipped: false },
            { label: 'Budget', data: items.map(b => b.limit_amount), backgroundColor: 'rgba(0,0,0,0.06)', borderRadius: 4, borderSkipped: false },
          ],
        },
        options: {
          indexAxis: 'y',
    ...
    async save() {
      const body = { ...this.form, limit_amount: parseFloat(this.form.limit_amount) };
      ...
      try {
        ...
        this.showForm = false;
        this.editId = null;
        this.form = { category: '', month_year: '', limit_amount: '', alert_threshold: 0.8 };
        await this.load();
      } catch (e) {
    ...
    startEdit(rule) {
      this.editId = rule.id;
      this.form = { ...rule, month_year: rule.month_year || '', limit_amount: String(rule.limit_amount) };
      this.showForm = true;
    },

    fmtAbs, barWidth, barClass, badgeClass,
  };
}
  ...
  return {
    ...
    renderChart() {
      ...
      this.chart = new Chart(el, {
        ...
        options: {
          plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` $${ctx.parsed.y.toLocaleString('en-US', { minimumFractionDigits: 2 })}` } } },
          scales: {
            y: { beginAtZero: false, grid: { color: 'rgba(0,0,0,0.04)' }, ticks: { callback: v => '$' + (v >= 1000 ? (v/1000).toFixed(0) + 'k' : v) } },
    ...
    async addSnapshot() {
      const body = { account_id: parseInt(this.snapForm.account_id), balance: parseFloat(this.snapForm.balance) };
      if (this.snapForm.snapshot_date) body.snapshot_date = this.snapForm.snapshot_date;
  ...
  return {
    ...
    async ask() {
      ...
      try {
        ...
        this.result = res;
        this.history.unshift({ q, a: res.answer, source: res.source, data: res.data });
      } catch (e) {
...
function recurringApp() {
  return {
    ...
    _toMonthly(amount, freq) {
      const map = { weekly: 4.33, biweekly: 2.17, monthly: 1, annual: 1 / 12, irregular: 1 };
...
function forecastApp() {
  return {
    ...
    async load() {
      this.loading = true;
      try {
        this.forecast = await api.get(`/api/recurring/forecast?days=${this.days}&include_detected=true`);
        this.$nextTick(() => this.renderChart());
      } catch (e) {
        console.error(e);
      }
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                ...
                label: ctx => ` Net flow: ${ctx.parsed.y >= 0 ? '+' : ''}$${ctx.parsed.y.toFixed(2)}`,
                afterBody: (items) => {
